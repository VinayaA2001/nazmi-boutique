// app/auth/forgotpassword/ForgotPasswordClient.tsx
"use client";
...
export default function ForgotPasswordClient() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (resp.ok) {
        // ✅ Show clear "Mail sent" message
        setMessage(
          data?.message ||
            "Mail sent. If an account with that email exists, please check your inbox for reset instructions."
        );

        // If backend returns reset_link in dev mode, append it (only visible to you)
        if (data?.reset_link) {
          setMessage(
            (data?.message ||
              "Mail sent. Password reset link (dev): ") + `\n${data.reset_link}`
          );
        }
      ...
      if (resp.status === 429) {
        ...
        setError(
          data?.message || "Invalid request. Please check the email and try again."
        );
