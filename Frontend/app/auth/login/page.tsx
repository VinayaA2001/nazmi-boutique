...
import { postJSON, API } from "@/lib/api"; // from your unified api.ts

// Helper to set readable cookies for middleware
...
export default function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await postJSON<{ message: string; token: string; user: any }>(
        `/api/auth/login`,
        // backend accepts email or phone; send as email for both cases
        { email: identifier, password }
      );

      // Save token for your app usage too
      try {
        localStorage.setItem("auth_token", data.token);
        // Align with AuthContext which reads 'auth_user'
        localStorage.setItem("auth_user", JSON.stringify(data.user ?? {}));
      } catch {}

      // 👇 Cookies your middleware.ts reads
      setCookie("auth_token", data.token, 7);
      setCookie("user", JSON.stringify(data.user ?? {}), 7);

      router.replace(redirect); // e.g. /account
    } catch (e: any) {
  ...
  async function sendForgot() {
    setForgotMsg(null);
    try {
      // Use same-origin proxy route to avoid CORS issues
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-md rounded-xl p-6 shadow-lg">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setForgotOpen(false)} className="text-2xl leading-none">
                ×
              </button>
            </div>
