...
function CheckoutContent() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!r.ok) return;
        const data = await r.json().catch(() => null) as { addresses?: any[], defaultId?: string } | null;
  ...
  useEffect(() => {
    try {
      const raw = (typeof window !== 'undefined') ? localStorage.getItem('last_shipping_address') : null;
      if (!raw) return;
  ...
  const payWithRazorpay = async () => {
    ...
    try {
      ...
      localStorage.setItem("last_shipping_address", JSON.stringify({
      ...
      const rzp = new window.Razorpay({
        ...
        theme: { color: "#000000" },
        modal: { ondismiss: async () => { setLoading(false); try { await fetch(`/api/payments/attempt-log`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ order_id: orderId, status: "cancelled", razorpay_order_id: rpJson.razorpay_order_id }) }); } catch {} } },
        retry: { enabled: false },
        timeout: 900,
        handler: async (resp: any) => {
          try {
            ...
            // Save address to profile (only if logged in)
            if (token) {
              try {
            ...
            try {
              await fetch(`/api/payments/attempt-log`, {
                ...
                body: JSON.stringify({ order_id: orderId, status: "failed", razorpay_order_id: resp?.razorpay_order_id, razorpay_payment_id: resp?.razorpay_payment_id, reason: err?.message })
              });
            } catch {}
      ...
      rzp.on("payment.failed", (r: any) => {
        ...
        try {
          fetch(`/api/payments/attempt-log`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ order_id: orderId, status: "failed", razorpay_order_id: rpJson.razorpay_order_id, reason: msg }),
          });
        } catch {}
  ...
  const DamagedProductsPolicy = () => (
    <div className="border-t pt-6 mt-6">
      <div className="space-y-4">
        ...
        <div className="text-center">
          <p className="text-xs text-gray-600">Help: <span className="font-semibold">+91-9995947709</span></p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-5">
        ...
        <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
          {cartItems.map((item) => (
            <div key={item.id} className="flex items-center gap-3 border-b last:border-b-0 pb-3 last:pb-0">
              <div className="w-14 h-14 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
                <Image src={item.image || "/images/poster1.png"} alt={item.name} width={56} height={56} className="w-full h-full object-cover" />
              </div>
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] text-gray-400">{inr(item.price)} / item</p>
              </div>
            </div>
          ))}
        </div>
        <div className="border-t mt-3 pt-3 space-y-1.5 text-sm">
          <div className="flex justify-between"><span className="text-gray-600">Subtotal</span><span className="font-medium text-gray-900">{inr(subtotal)}</span></div>
          <div className="flex justify-between">
            <span className="text-gray-600">Shipping</span>
            <span className={`font-medium ${shippingFee === 0 ? "text-green-700" : "text-gray-900"}`}>{shippingFee === 0 ? "FREE" : inr(shippingFee)}</span>
          </div>
      ...
      <div className="border-b bg-white">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 text-center lg:text-left">
            <h1 className="text-sm sm:text-base font-semibold text-gray-900">Checkout – Nazmi Boutique</h1>
            <p className="text-[11px] text-gray-500">Secure payment powered by Razorpay</p>
          </div>
          <button className="ml-auto inline-flex items-center gap-2 text-xs font-medium text-gray-700 lg:hidden" onClick={() => setShowOrderSummary(true)}>
            <span>View summary</span>
            <span className="px-2 py-1 rounded-full bg-gray-100 text-[11px]">{inr(grandTotal)}</span>
          </button>
        </div>
      </div>
      ...
      <main className="max-w-6xl mx-auto px-4 pt-4 lg:pt-8 grid lg:grid-cols-[minmax(0,2fr)_minmax(0,1.25fr)] gap-6 lg:gap-8">
        ...
        <section>
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
            <h2 className="text-base font-semibold text-gray-900 mb-4">Shipping Details</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div className="sm:col-span-2">
                <label className="block text-xs font-medium text-gray-700 mb-1">Full Name</label>
                <input type="text" value={shipping.name} onChange={(e) => setShipping((s) => ({ ...s, name: e.target.value }))} className="w-full p-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black focus:border-transparent" placeholder="Enter your full name" />
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Email Address</label>
                <input type="email" value={shipping.email} onChange={(e) => setShipping((s) => ({ ...s, email: e.target.value }))} className="w-full p-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black focus:border-transparent" placeholder="example@gmail.com" />
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Phone Number</label>
                <input type="tel" value={shipping.phone} onChange={(e) => setShipping((s) => ({ ...s, phone: e.target.value }))} className="w-full p-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black focus:border-transparent" placeholder="10-digit mobile number" />
              </div>

              <div className="sm:col-span-2">
                <label className="block text-xs font-medium text-gray-700 mb-1">Address</label>
                <input type="text" value={shipping.address1} onChange={(e) => setShipping((s) => ({ ...s, address1: e.target.value }))} className="w-full p-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black focus:border-transparent" placeholder="House / Building / Street / Landmark" />
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">City</label>
                <input type="text" value={shipping.city} onChange={(e) => setShipping((s) => ({ ...s, city: e.target.value }))} className="w-full p-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black focus:border-transparent" placeholder="City" />
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">State</label>
                <input type="text" value={shipping.state} onChange={(e) => setShipping((s) => ({ ...s, state: e.target.value }))} className="w-full p-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black focus:border-transparent" placeholder="State" />
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Pincode</label>
                <input type="text" value={shipping.pincode} onChange={(e) => setShipping((s) => ({ ...s, pincode: e.target.value }))} className="w-full p-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black focus:border-transparent" placeholder="6-digit pincode" />
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Country</label>
                <input type="text" value={shipping.country} onChange={(e) => setShipping((s) => ({ ...s, country: e.target.value }))} className="w-full p-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black focus:border-transparent" placeholder="Country" />
              </div>
            </div>
            ...
            <div className="mt-5">
              ...
              <p className="text-[11px] text-gray-500 mt-2 text-center">Do not refresh or close this page during payment.</p>
            </div>
          </div>
        </section>
        ...
        <div className="fixed inset-0 z-40 bg-black/40 flex justify-center items-end lg:hidden">
          <div className="bg-white rounded-t-2xl w-full max-h-[80vh] p-4 shadow-xl">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setShowOrderSummary(false)} className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center">
